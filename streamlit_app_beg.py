...
if 'user' not in st.session_state:
    user_df=get_data('select * from users_beg')
    ...
    if user_name != 'please select':
      if user_name == 'new':
        ...
        if new_user_name in user_list:
          ...
          run(f"INSERT INTO users_beg (user_id, user_name) VALUES ({user_id}, '{new_user_name}')")
          run(f'drop table if exists history_user_beg_{user_id}')
          run(f"""
          CREATE TABLE history_user_beg_{user_id} (
        ...
        user_id=get_data(f"select user_id from users_beg where user_name='{user_name}'").user_id.values[0]
        st.session_state.user_id=user_id
        st.rerun()
else:
  ...
  if 'df_words' not in st.session_state:
    ...
    st.session_state.df_box=get_data(f'select * from history_user_beg_{user_id}')
    st.rerun()
